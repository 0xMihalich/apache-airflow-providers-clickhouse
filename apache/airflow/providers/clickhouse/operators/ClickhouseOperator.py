...
from apache.airflow.providers.clickhouse.hooks.ClickhouseHook import ClickhouseHook  # noqa: E501


class ClickhouseOperator(BaseOperator):
    """ClickhouseOperator class."""
    ...
    def execute(self, context) -> None:
        ...
        client = ClickhouseHook(
            click_conn_id=self.click_conn_id,
        )
        self.log.info("Load client: %s", client)
        rows, col_definitions = client.run(
            sql=self.sql, parameters=self.parameters, with_column_types=True
        )
